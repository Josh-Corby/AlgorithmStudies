...
string Encoder::to_base64(const vector<unsigned char>& data)
{
    string result;
    ...
    while(j++ < data.size() / 3)
    {
        // put values of data[i], data[i+1], data[i+2] into 24 bit integer
        unsigned int value = data[i] << 16 | (data[i+1] << 8) | data[i+2]; 
        i+=3;

        // seperate 24 bit into 4 6 bit ints
...
struct converter
{
    ...
    static string from_range(std::vector<unsigned char> const & data)
    {
        string result;
        copy(
            begin(data), end(data),
